...
This runbook defines wallet consent, claim execution, sponsorship, and operator recovery. Transaction history and activation status are maintained in the [release record](v931-release-record.json).
...
The worker runs on the 14th of each month, hourly from 09:23 through 23:23 UTC, to process eligible opt-ins and retry failures. Live submissions are blocked on other UTC dates; simulations remain available. Unfinished claims wait until the next month. Manual claims,
...
Default submission limit is 300 wallet batches per run, including failed batches.
