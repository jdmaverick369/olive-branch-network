...
export function useTotalStakedAcrossPools({
  ...
  const effectiveAddress = (walletAddress ?? address) as Address | undefined;
    ...
    async function fetchAll() {
      ...
      try {
        ...
        const contracts = Array.from({ length }, (_, pid) => ({
          ...
          args: [BigInt(pid), effectiveAddress] as const,
        }));
  ...
  }, [effectiveAddress, publicClient, stakingAddress, stakingAbi, getUserInfoName, pollMs]);
