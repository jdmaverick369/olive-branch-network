...
import "./MonthlyAutoClaimUpgradeable.sol";
 ...
 */
contract OBNStakingPoolsV931 is
    Initializable,
    UUPSUpgradeable,
    ReentrancyGuardUpgradeable,
    OwnableUpgradeable,
    IStakingPools,
    MonthlyAutoClaimUpgradeable
{
    ...
    /// @notice Atomically activate V9.3.1 during the Timelock upgrade.
    function initializeV931(address executor) external reinitializer(931) onlyOwner {
        require(upgradeBlock != 0, "V9.3 migration required");
        require(executor != address(0), "executor=0");
        version = "9.3.1";
        _setAutoClaimExecutor(executor);
    }

    function _hasAutoClaimStake(address user) internal view override returns (bool) {
        return activePoolCount[user] > 0;
    }

    function _executeAutoClaim(uint256 pid, address user) internal override returns (uint256) {
        uint256 beforeClaim = totalClaimedByUser[user];
        _claimTo(pid, user);
        return totalClaimedByUser[user] - beforeClaim;
    }
